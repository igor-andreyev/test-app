...
APP?=test-app
PROJECT=github.com/igor-andreyev/test-app
...
TESTAPP_LOCAL_HOST?=0.0.0.0
TESTAPP_LOCAL_PORT?=8080
TESTAPP_LOG_LEVEL?=0
...
run: build
	@echo "+ $@"
	@docker run --name ${CONTAINER_NAME} -p ${TESTAPP_LOCAL_PORT}:${TESTAPP_LOCAL_PORT} \
		-e "TESTAPP_LOCAL_HOST=${TESTAPP_LOCAL_HOST}" \
		-e "TESTAPP_LOCAL_PORT=${TESTAPP_LOCAL_PORT}" \
		-e "TESTAPP_LOG_LEVEL=${TESTAPP_LOG_LEVEL}" \
